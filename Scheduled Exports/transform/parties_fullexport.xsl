...
<xsl:template match="/">{"parties": [
    <xsl:for-each select="table[@name='eparties']/tuple"><xsl:sort select="atom[@name='irn']" data-type="number"/>{
        ...
        "otherNames": [<xsl:for-each select="table[@name='OtherPersonNames']/tuple">"<xsl:value-of select="replace(replace(atom[@name='NamOtherNames'], '&quot;', '\\&quot;'), '&#09;', '\\&#09;')"/>"<xsl:if test="position() != last()">, </xsl:if></xsl:for-each>],
        ...
        "birthEarliestDate": <xsl:choose><xsl:when test="atom[@name='BioBirthEarliestDate'] != ''">"<xsl:value-of select="atom[@name='BioBirthEarliestDate']"/>"</xsl:when><xsl:otherwise>null</xsl:otherwise></xsl:choose>,
        "birthLatestDate": <xsl:choose><xsl:when test="atom[@name='BioBirthLatestDate'] != ''">"<xsl:value-of select="atom[@name='BioBirthLatestDate']"/>"</xsl:when><xsl:otherwise>null</xsl:otherwise></xsl:choose>,
        ...
        "deathEarliestDate": <xsl:choose><xsl:when test="atom[@name='BioDeathEarliestDate'] != ''">"<xsl:value-of select="atom[@name='BioDeathEarliestDate']"/>"</xsl:when><xsl:otherwise>null</xsl:otherwise></xsl:choose>,
        "deathLatestDate": <xsl:choose><xsl:when test="atom[@name='BioDeathLatestDate'] != ''">"<xsl:value-of select="atom[@name='BioDeathLatestDate']"/>"</xsl:when><xsl:otherwise>null</xsl:otherwise></xsl:choose>,
        ...
        "otherOrganizationNames": [<xsl:for-each select="table[@name='OtherOrgNames']/tuple">"<xsl:value-of select="replace(replace(atom[@name='NamOrganisationOtherNames'], '&quot;', '\\&quot;'), '&#09;', '\\&#09;')"/>"<xsl:if test="position() != last()">, </xsl:if></xsl:for-each>],
