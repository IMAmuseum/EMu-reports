...
<xsl:template match="/">{"objects": [
    <xsl:for-each select="table[@name='ecatalogue']/tuple"><xsl:sort select="atom[@name='irn']" data-type="number"/>{
        ...
        "currentLocType": <xsl:choose><xsl:when test="atom[@name='LocMovementType'] != ''">"<xsl:value-of select="replace(replace(atom[@name='LocMovementType'], '&quot;', '\\&quot;'), '&#09;', '\\&#09;')"/>"</xsl:when><xsl:otherwise>null</xsl:otherwise></xsl:choose>,
        "currentLocation": {<xsl:if test="table[@name='CurLoc']/tuple/tuple[@name='LocCurrentLocationRef']/atom[@name='irn'] != ''">
            ...
            "locationType": <xsl:choose><xsl:when test="table[@name='CurLoc']/tuple/tuple[@name='LocCurrentLocationRef']/atom[@name='LocLocationType'] != ''">"<xsl:value-of select="replace(replace(table[@name='CurLoc']/tuple/tuple[@name='LocCurrentLocationRef']/atom[@name='LocLocationType'], '&quot;', '\\&quot;'), '&#09;', '\\&#09;')"/>"</xsl:when><xsl:otherwise>null</xsl:otherwise></xsl:choose>,
