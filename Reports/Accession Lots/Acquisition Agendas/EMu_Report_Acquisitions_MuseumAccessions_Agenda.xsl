            ...
            <td class="heading">
                <xsl:choose>
                    <xsl:when test="ancestor::tuple/table[@name='AccAccessionLotRef']/tuple[2] or ancestor::table[@name='eaccessionlots']/tuple[2]">
                        <b><span style="text-decoration: underline;">Museum Accessions for the <xsl:if test="starts-with(ancestor::tuple/atom[@name='LotDepartment'], '901.1-European')">European Painting and Sculpture 1800-1945</xsl:if>
                    ...
                    <xsl:otherwise>
                        <b><span style="text-decoration: underline;">Museum Accession for the <xsl:if test="starts-with(ancestor::tuple/atom[@name='LotDepartment'], '901.1-European')">European Painting and Sculpture 1800-1945</xsl:if>
                            <xsl:if test="starts-with(ancestor::tuple/atom[@name='LotDepartment'], '901.2-American')">American Painting and Sculpture 1800-1945</xsl:if>
                            <xsl:if test="starts-with(ancestor::tuple/atom[@name='LotDepartment'], '902.1-European')">European Painting and Sculpture Before 1800</xsl:if>
                            <xsl:if test="starts-with(ancestor::tuple/atom[@name='LotDepartment'], '903.1-Decorative')">Decorative Arts</xsl:if>
                            <xsl:if test="starts-with(ancestor::tuple/atom[@name='LotDepartment'], '903.2-Design')">Design Arts</xsl:if>
                ...
                <xsl:text>Total Value of Museum Accessions: $</xsl:text><xsl:value-of select="format-number(sum(ancestor::table[@name='eaccessionlots']/tuple/table[@name='AccAccessionLotRef']/tuple/atom[@name='ValValuationAmount'][number(.)=number(.)]), '###,###,###,###,###.00')"/>
            </td>
        </tr>
    </xsl:template>
    <!--
