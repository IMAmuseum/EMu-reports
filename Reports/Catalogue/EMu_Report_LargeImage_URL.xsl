...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="table[@name='ecatalogue']">
        <html>
            ...
            <head>
                <title>Newfields Collection Report</title>
            </head>
        ...
        <xsl:for-each select="tuple">
            ...
            <xsl:variable name="url">http://collection.imamuseum.org/artwork/<xsl:value-of select="translate($dagwood, ' ', '')"/></xsl:variable>
            <xsl:variable name="image_url"><xsl:value-of select="$url"/>/0_full.jpg</xsl:variable>
            <xsl:choose>
...
<!--Image-->
                <xsl:element name="img">
                    ...
                    <xsl:attribute name="src"><xsl:if test="$dagwood != ''"><xsl:value-of select="$image_url"/></xsl:if></xsl:attribute>
                </xsl:element>
                ...
                <table class="table table-condensed">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><xsl:choose><xsl:when test="$dagwood != ''"><xsl:element name="a"><xsl:attribute name="href"><xsl:value-of select="$url"/></xsl:attribute><xsl:attribute name="target">blank</xsl:attribute><xsl:value-of select="$url"/></xsl:element></xsl:when><xsl:otherwise>Not Available</xsl:otherwise></xsl:choose></td>
                        </tr>
                    </tbody>
                </table>
            </center>             
