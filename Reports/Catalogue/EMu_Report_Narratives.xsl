...
p
{
    ...
    <xsl:template name="body">
        <center>
            ...
            <br/>
            <span style="font-weight: bold; font-size: 16pt;">Narratives Report</span>
        </center>
    ...
    <!--
        Narratives Table
    -->
        <xsl:if test="table[@name='Narratives']/tuple[atom[@name='DesPurpose'] = 'Object Label'] or table[@name='Narratives']/tuple[atom[@name='DesPurpose'] = 'Exhibition Label']">
        <tr class="heading">
            <td class="heading">Associated Narratives</td>
        </tr>
        <xsl:for-each select="table[@name='Narratives']/tuple">
        <xsl:if test="atom[@name='DesPurpose'] = 'Object Label' or atom[@name='DesPurpose'] = 'Exhibition Label'">
        <tr class="narratives">
            <td>
                <table class="picture">
                    <tr class="picture">
                        <td class="atomvalue" style="text-align: left;">
                            <b>Title: </b><xsl:value-of select="atom[@name='NarTitle']"/>
                        </td>
                        <td class="atomvalue" style="text-align: center;">
                            <b>Purpose: </b><xsl:value-of select="atom[@name='DesPurpose']"/>
                        </td>
                        <td class="atomvalue" style="text-align: right;">
                            <b>Online?: </b><xsl:value-of select="atom[@name='AdmPublishWebNoPassword']"/>
                        </td>
                    </tr>
                </table>
            </td>
        ...
        <tr class="narratives">
            <td class="atomvalue"><xsl:if test="table[@name='Authors']">
            <b>Author(s): </b><xsl:for-each select="table[@name='Authors']/tuple"><xsl:value-of select="atom[@name='NamFullName']"/><xsl:if test="position() != last()"><xsl:text>, </xsl:text></xsl:if></xsl:for-each><br/></xsl:if><xsl:if test="table[@name='Events']">
            <b>Associated Event(s): </b><xsl:for-each select="table[@name='Events']/tuple"><xsl:value-of select="atom[@name='EveEventNumber']"/><xsl:text> - </xsl:text><xsl:value-of select="atom[@name='EveEventTitle']"/><xsl:if test="position() != last()"><xsl:text>; </xsl:text></xsl:if></xsl:for-each><br/></xsl:if>
            <b>Narrative: </b><br/><xsl:value-of select="atom[@name='NarNarrative']"/></td>
        </tr>
        </xsl:if><xsl:if test="position() != last()"><br/></xsl:if>
        </xsl:for-each>
        </xsl:if>
    </xsl:template>
</xsl:stylesheet>